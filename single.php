            ...
            <div class="container">
                <div class="single-hero__inner">
                    <div class="single-hero__inner__bread-crumbs">
                        ...
                        <?php elseif (in_category(198)) : ?>
                            <a href="/health-insurance-options-by-state">Health Insurance Options By State</a>
                            <img src="<?php echo get_template_directory_uri(); ?>/static/images/right-caret.png"
                                 alt="right caret">
                ...
                <div class="single-content">
                    ...
                    <div class="single-content__post-content">
                        ...
                        <section class="single-content__post-content__credits">
                            ...
                            <?php if (!empty($reviewed_by)) : ?>
                                <?php
                                $reviewer_id = is_array($reviewed_by) ? $reviewed_by['ID'] : (int)$reviewed_by;

                                // Normalize ACF image (array | ID | URL)
                                $reviewer_image_raw = get_field('user_photo', 'user_' . $reviewer_id, false);
                                $reviewer_image_url = '';
                                $reviewer_image_alt = get_the_author_meta('display_name', $reviewer_id);

                                if (is_numeric($reviewer_image_raw)) {
                                    $reviewer_image_url = wp_get_attachment_image_url((int)$reviewer_image_raw, 'full');
                                    $alt = get_post_meta((int)$reviewer_image_raw, '_wp_attachment_image_alt', true);
                                    if ($alt) $reviewer_image_alt = $alt;
                                } elseif (is_array($reviewer_image_raw) && !empty($reviewer_image_raw['url'])) {
                                    $reviewer_image_url = $reviewer_image_raw['url'];
                                    if (!empty($reviewer_image_raw['alt'])) $reviewer_image_alt = $reviewer_image_raw['alt'];
                                } elseif (is_string($reviewer_image_raw)) {
                                    $reviewer_image_url = $reviewer_image_raw;
                                }
                                ...
                                <div class="single-content__post-content__credits__column">
                                    <div class="single-content__post-content__credits__column__user">
                                        <img
                                            class="single-content__post-content__credits__column__user__photo"
                                            src="<?php echo esc_url($reviewer_image_url ?: get_template_directory_uri() . '/static/images/default-author.png'); ?>"
                                            alt="<?php echo esc_attr($reviewer_image_alt); ?>">
